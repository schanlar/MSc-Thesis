...
%In the previous chapter, we have discussed the evolution of two helium stars with initial masses 1.8\,$\rm M_{\odot}$ and 2.5\,$\rm M_{\odot}$ that evolve to near-Chandrasekhar (C)ONeMg cores, and subsequently disrupt in a thermonuclear explosion. In this chapter, we further explore this novel SN\,Ia progenitor channel by performing detailed numerical calculations using \mesa in order to explore the influence of initial metallicity, and overshoot mixing on these degenerate cores. 
%More specifically, we evolved 252 single, non-rotating helium stars  in the initial mass range $\rm 0.8-3.5\, M_{\odot}$, adopting values for initial metallicity that range from low ($Z=10^{-4}$) to solar ($Z=0.02$), and overshoot efficiency $\rm f_{OV} = [0.014, 0.016]$. We find that overshooting across the convective boundary of the core and the envelope---during core helium burning---results in larger convective cores, prolonging the helium main-sequence phase. There seems to be a negative correlation between initial metallicity and core growth. In most cases, degenerate near-Chandrasekhar ONe cores are being formed, that retain small amounts of unburnt carbon. We show that in virtual lack of residual carbon, these cores will most likely evolve towards an electron capture supernova
...
\end{subfigure}
\hfill
\caption{Abundance profiles of our fiducial models during various evolutionary stages. Panels (a) and (b) refer to the abundace profiles when when $\rm \log(\rho_c / gr \ cm^{-3}) \approx 9.0$ (indicated by the grey circle in Fig.\,\ref{fig:RhoT}). In panel (d), the core reaches the density threshold for e-captures on Ne nuclei to commence, leading to formation of $\rm ^{20}O$, and most likely to an ECSN.}
\label{fig:abundance_profiles}
\end{figure}
