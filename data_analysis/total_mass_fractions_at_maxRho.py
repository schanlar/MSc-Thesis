...
for path in glob.glob(os.path.join(grid, '*')):
	...
	if model in models:
		...
		carbon_fraction = (p.data('c12')[np.where(p.data('mass') <= p.data('mass')[0])] * dm[np.where(p.data('mass') <= p.data('mass')[0])]).sum() / p.data('mass')[0]
		oxygen_fraction = (p.data('o16')[np.where(p.data('mass') <= p.data('mass')[0])] * dm[np.where(p.data('mass') <= p.data('mass')[0])]).sum() / p.data('mass')[0]
		neon_fraction = (p.data('ne20')[np.where(p.data('mass') <= p.data('mass')[0])] * dm[np.where(p.data('mass') <= p.data('mass')[0])]).sum() / p.data('mass')[0]	
